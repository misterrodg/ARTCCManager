...
const LOCAL_NASR_DOWNLOAD_URL = "/nasr/download";
const LOCAL_NASR_DECOMPRESS_URL = "/nasr/decompress";

const NASRDownload = ({ faaDataObject, handleErrorMessage }) => {
  ...
  async function getDownload() {
    ...
    try {
      const res = await axios.post(`${LOCAL_NASR_DOWNLOAD_URL}`, postData, {
        headers: {
  ...
  async function decompressDownload() {
    ...
    try {
      const res = await axios.post(`${LOCAL_NASR_DECOMPRESS_URL}`, postData, {
        headers: {
          "x-access-token": "token-value",
        },
      });
